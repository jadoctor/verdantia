  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos, datosadjuntosruta, datosadjuntosnombreoriginal, datosadjuntostitulo, datosadjuntosresumen, datosadjuntosapuntes, datosadjuntosportada, datosadjuntosautores, datosadjuntosidentificacion, datosadjuntosactivo
       FROM datosadjuntos 
       WHERE xdatosadjuntosidvariedadesvegetales = ? 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const filename = `variedad_${idvariedadesvegetales}_${Date.now()}.pdf`;
    const storagePath = `uploads/variedades_pdfs/${filename}`;
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidvariedadesvegetales = ? AND datosadjuntostipo = 'documento' AND datosadjuntosactivo = 1",
      [idvariedadesvegetales]
    );
    ...
    const [result] = await pool.query(
      ...
      [file.name, storagePath, total + 1, idvariedadesvegetales, fileSize, generatedTitle, generatedSummary, generatedApuntes, generatedAutores, generatedIdentificacion]
    );
    ...
    console.error('[Variedad PDFs API] Upload error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (base64Cover) {
      ...
      if (!docTitle) {
        const [pdfRows] = await pool.query(
          "SELECT datosadjuntostitulo FROM datosadjuntos WHERE iddatosadjuntos = ? AND xdatosadjuntosidvariedadesvegetales = ?",
          [pdfId, idvariedadesvegetales]
        );
      ...
      const storagePath = `uploads/variedades_pdfs_covers/${cleanTitle}_${pdfId}_${Date.now()}.jpg`;
      const { uploadToStorage } = await import('@/lib/firebase/storage');
    ...
    updateQuery += " WHERE iddatosadjuntos = ? AND xdatosadjuntosidvariedadesvegetales = ? AND datosadjuntostipo = 'documento'";
    updateParams.push(pdfId, idvariedadesvegetales);

    await pool.query(updateQuery, updateParams);

    return NextResponse.json({ success: true, portada: finalPortada });
  } catch (error: any) {
    console.error('[Variedad PDFs PUT API] error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
