...
const ALLOWED_UPLOAD_PREFIXES = ['uploads/temp/', 'uploads/plantasparte/'];
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const idplantasparte = resolvedParams.id;
  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidplantasparte = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const reqPlantasParteNombre =
      typeof body.plantasparteNombre === 'string' ? body.plantasparteNombre : '';
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidplantasparte = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1",
      [idplantasparte]
    );
    ...
    let plantasparteNombre = reqPlantasParteNombre as string;
    if (!plantasparteNombre) {
      const [ptResult] = await pool.query("SELECT plantaspartenombre FROM plantasparte WHERE idplantasparte = ?", [idplantasparte]);
      plantasparteNombre = (ptResult as any[])[0]?.plantaspartenombre || 'Parte de planta';
    }
    ...
    if (apiKey) {
      try {
        ...
        const payload = {
          contents: [{
            parts: [
              { text: `Analiza esta imagen relacionada con la parte de planta '${plantasparteNombre}' de un huerto. Responde ÚNICAMENTE con un objeto JSON válido con esta estructura exacta: {"seo_alt": "descripción visual en español útil para SEO, máximo 80 caracteres"}` },
        ...
        console.error('[PlantasParte Photos API] Error llamando a Gemini para SEO:', e);
      }
    }

    const textToSlugify = seoAlt || plantasparteNombre;
    ...
    const storagePath = `uploads/plantasparte/${filename}`;
    const thumbFilename = `thumb-${filename}`;
    const thumbStoragePath = `uploads/plantasparte/${thumbFilename}`;
    ...
    const [result] = await pool.query(
      `INSERT INTO datosadjuntos (
        ...
        datosadjuntosactivo, datosadjuntosfechacreacion, xdatosadjuntosidplantasparte,
        datosadjuntospesobytes, datosadjuntosresumen
      ) VALUES ('imagen', ?, ?, ?, ?, ?, 1, NOW(), ?, ?, ?)`,
      [
        fileType, fileName, storagePath, esPrimera,
        total + 1, idplantasparte, fileSize, initialResumen
      ]
    );
    ...
    console.error('[PlantasParte Photos API] Upload error:', error);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pool.query(
      'UPDATE datosadjuntos SET datosadjuntosactivo = 0, datosadjuntosfechaeliminacion = NOW() WHERE iddatosadjuntos = ? AND xdatosadjuntosidplantasparte = ?',
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'setPrimary') {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 0 WHERE xdatosadjuntosidplantasparte = ? AND datosadjuntostipo = "imagen"',
        [resolvedParams.id]
      );
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 1 WHERE iddatosadjuntos = ? AND xdatosadjuntosidplantasparte = ?',
        [photoId, resolvedParams.id]
      );
    ...
    if (action === 'updateMeta' && resumen) {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosresumen = ? WHERE iddatosadjuntos = ? AND xdatosadjuntosidplantasparte = ?',
