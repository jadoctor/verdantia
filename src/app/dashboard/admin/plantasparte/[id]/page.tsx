  ...
  const handleSubmit = async (e?: any) => {
    ...
    try {
      ...
      if (res.ok) {
        if (isNew) {
          const data = await res.json();
          router.push(`/dashboard/admin/plantasparte/${data.id}`);
        } else {
    ...
    <div style={{ padding: '24px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ marginBottom: '16px', display: 'flex', gap: '8px' }}>
        ...
        {searchParams.get('from') === 'pdfs' ? (
          ...
          <button onClick={() => router.push('/dashboard/admin/plantasparte')} style={{ background: 'white', border: '1px solid #cbd5e1', color: '#475569', padding: '6px 14px', borderRadius: '8px', cursor: 'pointer', fontWeight: 600, fontSize: '0.85rem' }}>
