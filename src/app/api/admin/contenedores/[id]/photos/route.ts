...
const ALLOWED_UPLOAD_PREFIXES = ['uploads/temp/', 'uploads/contenedores/'];
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const idcontenedores = resolvedParams.id;
  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidcontenedores = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const reqContenedorNombre =
      typeof body.contenedorNombre === 'string' ? body.contenedorNombre : '';
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidcontenedores = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1",
      [idcontenedores]
    );
    ...
    let contenedorNombre = reqContenedorNombre as string;
    if (!contenedorNombre) {
      const [contResult] = await pool.query("SELECT contenedoresnombre FROM contenedores WHERE idcontenedores = ?", [idcontenedores]);
      contenedorNombre = (contResult as any[])[0]?.contenedoresnombre || 'Contenedor';
    }
    ...
    if (apiKey) {
      try {
        ...
        const payload = {
          contents: [{
            parts: [
              { text: `Analiza esta imagen relacionada con el contenedor de cultivo agrícola '${contenedorNombre}'. Responde ÚNICAMENTE con un objeto JSON válido con esta estructura exacta: {"seo_alt": "descripción visual en español útil para SEO, máximo 80 caracteres"}` },
        ...
        console.error('[Contenedores Photos API] Error llamando a Gemini para SEO:', e);
      }
    }

    const textToSlugify = seoAlt || contenedorNombre;
    ...
    const storagePath = `uploads/contenedores/${filename}`;
    const thumbFilename = `thumb-${filename}`;
    const thumbStoragePath = `uploads/contenedores/${thumbFilename}`;
    ...
    const [result] = await pool.query(
      `INSERT INTO datosadjuntos (
        ...
        datosadjuntosactivo, datosadjuntosfechacreacion, xdatosadjuntosidcontenedores,
        datosadjuntospesobytes, datosadjuntosresumen
      ) VALUES ('imagen', ?, ?, ?, ?, ?, 1, NOW(), ?, ?, ?)`,
      [
        fileType, fileName, storagePath, esPrimera,
        total + 1, idcontenedores, fileSize, initialResumen
      ]
    );
    ...
    console.error('[Contenedor Photos API] Upload error:', error);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pool.query(
      'UPDATE datosadjuntos SET datosadjuntosactivo = 0, datosadjuntosfechaeliminacion = NOW() WHERE iddatosadjuntos = ? AND xdatosadjuntosidcontenedores = ?',
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'setPrimary') {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 0 WHERE xdatosadjuntosidcontenedores = ? AND datosadjuntostipo = "imagen"',
        [resolvedParams.id]
      );
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 1 WHERE iddatosadjuntos = ? AND xdatosadjuntosidcontenedores = ?',
        [photoId, resolvedParams.id]
      );
    ...
    if (action === 'updateMeta' && resumen) {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosresumen = ? WHERE iddatosadjuntos = ? AND xdatosadjuntosidcontenedores = ?',
