...
export default function ContenedorForm({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e?: any) => {
    ...
    if (res.ok) {
      if (isNew) {
        router.push('/dashboard/admin/contenedores');
      } else {
  ...
  return (
    <div style={{ padding: '20px', width: '100%', maxWidth: '100%', boxSizing: 'border-box' }}>
      <button onClick={() => router.push('/dashboard/admin/contenedores')} className="btn-back" style={{ marginBottom: '20px', display: 'inline-flex', alignItems: 'center', gap: '8px' }}>
