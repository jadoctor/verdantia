...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string, idPauta: string }> }) {
  ...
  try {
    ...
    if (isUsed) {
      ...
      await pool.query('UPDATE laborespauta SET laborespautaactivosino = 0 WHERE idlaborespauta = ? AND xlaborespautaidespeciesvegetales = ?', [idlaborespauta, resolvedParams.id]);
      return NextResponse.json({ success: true, message: 'La labor está en uso y ha sido marcada como inactiva.' });
    } else {
      // Hard delete
      await pool.query('DELETE FROM laborespauta WHERE idlaborespauta = ? AND xlaborespautaidespeciesvegetales = ?', [idlaborespauta, resolvedParams.id]);
      return NextResponse.json({ success: true });
    }
  } catch (error: any) {
