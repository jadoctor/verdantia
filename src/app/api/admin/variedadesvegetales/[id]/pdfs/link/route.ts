...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [result] = await pool.query(
      ...
      ['Documento_Web', finalUrl, total + 1, idvariedadesvegetales, safeTitle, safeSummary, safeApuntes, safeAutores, safeIdentificacion]
    );
    ...
    console.error('[Variedad PDFs Link API] error:', error);
