...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      if (apiKey && bytes.byteLength < 10 * 1024 * 1024) { // Only process if < 10MB to avoid timeouts/limits
        const payload = {
          contents: [{
            parts: [
              { text: `Analiza este documento PDF sobre la variedad agrícola. Eres un experto agrónomo. Devuelve EXCLUSIVAMENTE un JSON con tres propiedades: "nombre" (nombre súper corto del documento que aparecerá en el visor, ideal 3-4 palabras, máximo 40 caracteres), "resumen" (resumen corto de 1-2 líneas), y "apuntes" (un resumen técnico muy detallado de varios párrafos o viñetas estructuradas con datos concretos, plagas, metodologías y consejos vitales para el agricultor).` },
    ...
    const [result] = await pool.query(
      ...
      [file.name, storagePath, total + 1, idvariedades, fileSize, generatedTitle, generatedSummary, generatedApuntes]
    );
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (base64Cover) {
      ...
      const storagePath = `uploads/variedades_pdfs_covers/cover_${idvariedades}_${pdfId}_${Date.now()}.jpg`;
      const { uploadToStorage } = await import('@/lib/firebase/storage');
