...
const ALLOWED_UPLOAD_PREFIXES = ['uploads/temp/', 'uploads/tratamientos/'];
  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidtratamientos = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidtratamientos = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1",
      [idtratamientos]
    );
    ...
    let variedadNombre = reqTratamientoNombre as string;
    if (!variedadNombre) {
      const [variedadResult] = await pool.query("SELECT tratamientosnombre FROM tratamientos WHERE idtratamientos = ?", [idtratamientos]);
      variedadNombre = (variedadResult as any[])[0]?.tratamientosnombre || 'Tratamiento';
    }
    ...
    if (apiKey) {
      try {
        ...
        console.error('[Tratamientos Photos API] Error llamando a Gemini para SEO:', e);
      }
    }
    ...
    const storagePath = `uploads/tratamientos/${filename}`;
    const thumbFilename = `thumb-${filename}`;
    const thumbStoragePath = `uploads/tratamientos/${thumbFilename}`;
    ...
    const [result] = await pool.query(
      ...
      [
        ...
        total + 1, idtratamientos, fileSize, initialResumen
      ]
    );
    ...
    console.error('[Tratamiento Photos API] Upload error:', error);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pool.query(
      'UPDATE datosadjuntos SET datosadjuntosactivo = 0, datosadjuntosfechaeliminacion = NOW() WHERE iddatosadjuntos = ? AND xdatosadjuntosidtratamientos = ?',
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'setPrimary') {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 0 WHERE xdatosadjuntosidtratamientos = ? AND datosadjuntostipo = "imagen"',
        [resolvedParams.id]
      );
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 1 WHERE iddatosadjuntos = ? AND xdatosadjuntosidtratamientos = ?',
        [photoId, resolvedParams.id]
      );
    ...
    if (action === 'updateMeta' && resumen) {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosresumen = ? WHERE iddatosadjuntos = ? AND xdatosadjuntosidtratamientos = ?',
