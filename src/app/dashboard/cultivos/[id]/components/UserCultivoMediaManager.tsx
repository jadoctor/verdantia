...
export default function UserCultivoMediaManager({ cultivoId, userEmail, onMediaChange }: UserCultivoMediaManagerProps) {
  ...
  return (
    <div style={{ marginTop: '20px' }}>
      ...
      <div style={{ marginBottom: '20px', display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
        ...
        <button 
          ...
          style={{
            ...
            cursor: uploading ? 'not-allowed' : 'pointer', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px',
      ...
      {/* Galería */}
      {photos.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '40px', background: '#f8fafc', borderRadius: '12px', border: '2px dashed #cbd5e1', color: '#64748b' }}>
          <p>No hay fotos en la galería de este cultivo.</p>
        </div>
      ) : (
