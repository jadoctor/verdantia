...
// 2. Extract completadas block from 'tareas' and wrap it in its own tab
// From line 1215 to 1258
const completadasBlockTarget = `              {avisosCompletados.length > 0 && (
...
const completadasBlockReplacement = `            </div>
          );
        })()}
...
if (content.includes(completadasBlockTarget)) {
    content = content.replace(completadasBlockTarget, "");
    
    // Now insert completadasBlockReplacement just before the closing of the activeTab === 'tareas' block
    // It looks like `            </div>\n          );\n        })()}`
    ...
    if (content.includes(endTareasTarget)) {
       content = content.replace(endTareasTarget, completadasBlockReplacement);
    }
}
...
console.log('Tab injection script finished.');
