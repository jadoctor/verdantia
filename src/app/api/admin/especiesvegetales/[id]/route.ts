...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [rows]: any = await pool.query('SELECT * FROM especiesvegetales WHERE idespeciesvegetales = ?', [resolvedParams.id]);
    if (rows.length === 0) {
      return NextResponse.json({ error: 'Especie no encontrada' }, { status: 404 });
    }
    
    const especie = rows[0];
    const [fases]: any = await pool.query('SELECT xespeciesvegetalesfasesidfasescultivo as idFase, especiesfasesduraciondias as duracion FROM especiesfases WHERE xespeciesvegetalesfasesidespeciesvegetales = ?', [resolvedParams.id]);
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    queryParams.push(idespeciesvegetales);

    const query = `UPDATE especiesvegetales SET ${setClauses.join(', ')} WHERE idespeciesvegetales = ?`;
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [vars]: any = await pool.query(
      'SELECT 1 FROM variedadesvegetales WHERE xvariedadesvegetalesidespeciesvegetales = ? AND variedadesvegetalesesgenerica = 0 LIMIT 1',
      [idespeciesvegetales]
    );

    // Check if the species has any seeds associated with its varieties
    const [seeds]: any = await pool.query(
      'SELECT 1 FROM semillas s JOIN variedadesvegetales v ON s.xsemillasidvariedadesvegetales = v.idvariedadesvegetales WHERE v.xvariedadesvegetalesidespeciesvegetales = ? LIMIT 1',
      [idespeciesvegetales]
    );

    // Check if the species has any crops associated with its varieties
    const [cultivos]: any = await pool.query(
      'SELECT 1 FROM cultivos c JOIN variedadesvegetales v ON c.xcultivosidvariedadesvegetales = v.idvariedadesvegetales WHERE v.xvariedadesvegetalesidespeciesvegetales = ? LIMIT 1',
      [idespeciesvegetales]
    );

    // Check if the species belongs directly to a user
    const [owner]: any = await pool.query(
      'SELECT 1 FROM especiesvegetales WHERE idespeciesvegetales = ? AND xespeciesvegetalesidusuarios IS NOT NULL LIMIT 1',
      [idespeciesvegetales]
    );

    // Check if the species is customized by any user in especiesusuarios
    const [espUsers]: any = await pool.query(
      'SELECT 1 FROM especiesusuarios WHERE xespeciesvegetalesusuariosidespeciesvegetales = ? LIMIT 1',
      [idespeciesvegetales]
    );

    // Check if any varieties of the species are customized in variedadesvegetalesusuarios
    const [varUsers]: any = await pool.query(
      'SELECT 1 FROM variedadesvegetalesusuarios vu JOIN variedadesvegetales v ON vu.xvariedadesvegetalesusuariosidvariedadesvegetales = v.idvariedadesvegetales WHERE v.xvariedadesvegetalesidespeciesvegetales = ? LIMIT 1',
      [idespeciesvegetales]
    );

    // Check if the species has any associated pests
    const [afecciones]: any = await pool.query(
      'SELECT 1 FROM especiesafecciones WHERE xespeciesvegetalesafeccionesidespeciesvegetales = ? LIMIT 1',
      [idespeciesvegetales]
    );
    ...
    if (
      ...
      await pool.query('UPDATE especiesvegetales SET especiesvegetalesvisibilidadsino = 0 WHERE idespeciesvegetales = ?', [idespeciesvegetales]);
    ...
    await pool.query('DELETE FROM variedadesvegetales WHERE xvariedadesvegetalesidespeciesvegetales = ?', [idespeciesvegetales]);

    // Otherwise, perform physical delete
    await pool.query('DELETE FROM especiesvegetales WHERE idespeciesvegetales = ?', [idespeciesvegetales]);
