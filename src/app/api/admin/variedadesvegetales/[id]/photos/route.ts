...
const ALLOWED_UPLOAD_PREFIXES = ['uploads/temp/', 'uploads/variedades/'];
  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidvariedadesvegetales = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    let variedadNombre = reqVariedadNombre as string;
    if (!variedadNombre) {
      const [variedadResult] = await pool.query("SELECT variedadesvegetalesnombre FROM variedadesvegetales WHERE idvariedadesvegetales = ?", [idvariedadesvegetales]);
      variedadNombre = (variedadResult as any[])[0]?.variedadesvegetalesnombre || 'Variedad';
    }
    ...
    if (apiKey) {
      try {
        ...
        console.error('[Variedads Photos API] Error llamando a Gemini para SEO:', e);
      }
    }
    ...
    const storagePath = `uploads/variedades/${filename}`;
    const thumbFilename = `thumb-${filename}`;
    const thumbStoragePath = `uploads/variedades/${thumbFilename}`;
    ...
    const [result] = await pool.query(
      ...
      [
        ...
        total + 1, idvariedadesvegetales, fileSize, initialResumen
      ]
    );
    ...
    console.error('[Variedad Photos API] Upload error:', error);
