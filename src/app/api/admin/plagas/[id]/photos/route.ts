  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidplagas = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { rawStoragePath, plagaNombre: reqPlagaNombre } = body;
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidplagas = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1",
      [idplagas]
    );
    ...
    let plagaNombre = reqPlagaNombre as string;
    if (!plagaNombre) {
      const [plagaResult] = await pool.query("SELECT plagasnombre FROM plagas WHERE idplagas = ?", [idplagas]);
      plagaNombre = (plagaResult as any[])[0]?.plagasnombre || 'Plaga';
    }
    ...
    if (apiKey) {
      try {
        ...
        const payload = {
          contents: [{
            parts: [
              { text: `Analiza esta imagen relacionada con la plaga/enfermedad '${plagaNombre}'. Responde ÚNICAMENTE con un objeto JSON válido con esta estructura exacta: {"seo_alt": "descripción visual en español útil para SEO, máximo 80 caracteres"}` },
        ...
        console.error('[Plagas Photos API] Error llamando a Gemini para SEO:', e);
      }
    }

    const textToSlugify = seoAlt || plagaNombre;
    ...
    const storagePath = `uploads/plagas/${filename}`;
    const thumbFilename = `thumb-${filename}`;
    const thumbStoragePath = `uploads/plagas/${thumbFilename}`;
    ...
    const [result] = await pool.query(
      `INSERT INTO datosadjuntos (
        ...
        datosadjuntosactivo, datosadjuntosfechacreacion, xdatosadjuntosidplagas,
        datosadjuntospesobytes, datosadjuntosresumen
      ) VALUES ('imagen', ?, ?, ?, ?, ?, 1, NOW(), ?, ?, ?)`,
      [
        fileType, fileName, storagePath, esPrimera,
        total + 1, idplagas, fileSize, initialResumen
      ]
    );
    ...
    console.error('[Plagas Photos API] Upload error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pool.query(
      'UPDATE datosadjuntos SET datosadjuntosactivo = 0, datosadjuntosfechaeliminacion = NOW() WHERE iddatosadjuntos = ? AND xdatosadjuntosidplagas = ?',
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'setPrimary') {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 0 WHERE xdatosadjuntosidplagas = ? AND datosadjuntostipo = "imagen"',
        [resolvedParams.id]
      );
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 1 WHERE iddatosadjuntos = ? AND xdatosadjuntosidplagas = ?',
        [photoId, resolvedParams.id]
      );
    ...
    if (action === 'updateMeta' && resumen) {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosresumen = ? WHERE iddatosadjuntos = ? AND xdatosadjuntosidplagas = ?',
