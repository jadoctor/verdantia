...
const ALLOWED_UPLOAD_PREFIXES = ['uploads/temp/', 'uploads/afecciones/'];
  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidafecciones = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const reqAfeccionNombre =
      typeof body.variedadNombre === 'string' ? body.variedadNombre : '';
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidafecciones = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1",
      [idafecciones]
    );
    ...
    let variedadNombre = reqAfeccionNombre as string;
    if (!variedadNombre) {
      const [variedadResult] = await pool.query("SELECT afeccionesnombre FROM afecciones WHERE idafecciones = ?", [idafecciones]);
      variedadNombre = (variedadResult as any[])[0]?.afeccionesnombre || 'Afeccion';
    }
    ...
    if (apiKey) {
      try {
        ...
        const payload = {
          contents: [{
            parts: [
              { text: `Analiza esta imagen relacionada con la variedad biológica/botánica '${variedadNombre}'. Responde ÚNICAMENTE con un objeto JSON válido con esta estructura exacta: {"seo_alt": "descripción visual en español útil para SEO, máximo 80 caracteres"}` },
        ...
        console.error('[Afeccions Photos API] Error llamando a Gemini para SEO:', e);
      }
    }

    const textToSlugify = seoAlt || variedadNombre;
    ...
    const storagePath = `uploads/afecciones/${filename}`;
    const thumbFilename = `thumb-${filename}`;
    const thumbStoragePath = `uploads/afecciones/${thumbFilename}`;
    ...
    const [result] = await pool.query(
      `INSERT INTO datosadjuntos (
        ...
        datosadjuntosactivo, datosadjuntosfechacreacion, xdatosadjuntosidafecciones,
        datosadjuntospesobytes, datosadjuntosresumen
      ) VALUES ('imagen', ?, ?, ?, ?, ?, 1, NOW(), ?, ?, ?)`,
      [
        fileType, fileName, storagePath, esPrimera,
        total + 1, idafecciones, fileSize, initialResumen
      ]
    );
    ...
    console.error('[Afeccion Photos API] Upload error:', error);
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await pool.query(
      'UPDATE datosadjuntos SET datosadjuntosactivo = 0, datosadjuntosfechaeliminacion = NOW() WHERE iddatosadjuntos = ? AND xdatosadjuntosidafecciones = ?',
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'setPrimary') {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 0 WHERE xdatosadjuntosidafecciones = ? AND datosadjuntostipo = "imagen"',
        [resolvedParams.id]
      );
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 1 WHERE iddatosadjuntos = ? AND xdatosadjuntosidafecciones = ?',
        [photoId, resolvedParams.id]
      );
    ...
    if (action === 'updateMeta' && resumen) {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosresumen = ? WHERE iddatosadjuntos = ? AND xdatosadjuntosidafecciones = ?',
