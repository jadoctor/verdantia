  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos, datosadjuntosruta, datosadjuntosnombreoriginal, datosadjuntostitulo, datosadjuntosresumen, datosadjuntosapuntes, datosadjuntosportada, datosadjuntosautores, datosadjuntosidentificacion, datosadjuntosactivo
       FROM datosadjuntos 
       WHERE xdatosadjuntosidtratamientos = ? 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const filename = `variedad_${idtratamientos}_${Date.now()}.pdf`;
    const storagePath = `uploads/tratamientos_pdfs/${filename}`;
    ...
    const [result] = await pool.query(
      ...
      [file.name, storagePath, total + 1, idtratamientos, fileSize, generatedTitle, generatedSummary, generatedApuntes, generatedAutores, generatedIdentificacion]
    );
    ...
    console.error('[Tratamiento PDFs API] Upload error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (base64Cover) {
      ...
      if (!docTitle) {
        const [pdfRows] = await pool.query(
          "SELECT datosadjuntostitulo FROM datosadjuntos WHERE iddatosadjuntos = ? AND xdatosadjuntosidtratamientos = ?",
          [pdfId, idtratamientos]
        );
      ...
      const storagePath = `uploads/tratamientos_pdfs_covers/${cleanTitle}_${pdfId}_${Date.now()}.jpg`;
      const { uploadToStorage } = await import('@/lib/firebase/storage');
    ...
    updateQuery += " WHERE iddatosadjuntos = ? AND xdatosadjuntosidtratamientos = ? AND datosadjuntostipo = 'documento'";
    updateParams.push(pdfId, idtratamientos);

    await pool.query(updateQuery, updateParams);

    return NextResponse.json({ success: true, portada: finalPortada });
  } catch (error: any) {
    console.error('[Tratamiento PDFs PUT API] error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
