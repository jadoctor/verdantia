...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await pool.query('DELETE FROM especiesfases WHERE xespeciesvegetalesfasesidespeciesvegetales = ?', [idespeciesvegetales]);
    ...
    for (const [idFaseStr, duracion] of Object.entries(fases_duracion)) {
      ...
      if (!isNaN(idFase) && !isNaN(dias)) {
        insertValues.push('(?, ?, ?)');
        queryParams.push(idespeciesvegetales, idFase, dias);
      }
    }

    if (insertValues.length > 0) {
      await pool.query(
        `INSERT INTO especiesfases (xespeciesvegetalesfasesidespeciesvegetales, xespeciesvegetalesfasesidfasescultivo, especiesfasesduraciondias) VALUES ${insertValues.join(', ')}`,
        queryParams
      );
    }
