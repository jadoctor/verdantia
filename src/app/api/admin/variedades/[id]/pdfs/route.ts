  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos, datosadjuntosruta, datosadjuntosnombreoriginal, datosadjuntostitulo, datosadjuntosresumen, datosadjuntosapuntes, datosadjuntosportada, datosadjuntosautores, datosadjuntosidentificacion, datosadjuntosactivo
       FROM datosadjuntos 
       WHERE xdatosadjuntosidvariedades = ? 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const filename = `variedad_${idvariedades}_${Date.now()}.pdf`;
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidvariedades = ? AND datosadjuntostipo = 'documento' AND datosadjuntosactivo = 1",
      [idvariedades]
    );
    ...
    const [result] = await pool.query(
      ...
      [file.name, storagePath, total + 1, idvariedades, fileSize, generatedTitle, generatedSummary, generatedApuntes, generatedAutores, generatedIdentificacion]
    );
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (base64Cover) {
      ...
      if (!docTitle) {
        const [pdfRows] = await pool.query(
          "SELECT datosadjuntostitulo FROM datosadjuntos WHERE iddatosadjuntos = ? AND xdatosadjuntosidvariedades = ?",
          [pdfId, idvariedades]
        );
    ...
    updateQuery += " WHERE iddatosadjuntos = ? AND xdatosadjuntosidvariedades = ? AND datosadjuntostipo = 'documento'";
    updateParams.push(pdfId, idvariedades);
