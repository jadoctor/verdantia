...
interface AnalisisStatsProps {
  ...
  totalLines: number;
  maxLines: number;
  largestDashboardFile: string;
  isMobile?: boolean;
}

export function AnalisisStats({ totalDashboards, totalLines, maxLines, largestDashboardFile, isMobile = false }: AnalisisStatsProps) {
  return (
    <div style={{
      display: 'grid',
      ...
      <div style={{ background: 'linear-gradient(135deg, #059669, #34d399)', borderRadius: '14px', padding: isMobile ? '16px' : '20px 24px', color: 'white' }}>
        ...
        <div style={{ fontSize: '0.7rem', opacity: 0.85, marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={largestDashboardFile}>
