  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidespeciesvegetales = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidespeciesvegetales = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1",
      [idespeciesvegetales]
    );
    ...
    if (!especieNombre) {
      const [especieResult] = await pool.query("SELECT especiesvegetalesnombre FROM especiesvegetales WHERE idespeciesvegetales = ?", [idespeciesvegetales]);
      especieNombre = (especieResult as any[])[0]?.especiesvegetalesnombre || 'Especie';
    }
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (action === 'setPrimary') {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 0 WHERE xdatosadjuntosidespeciesvegetales = ? AND datosadjuntostipo = "imagen"',
        [resolvedParams.id]
      );
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosesprincipal = 1 WHERE iddatosadjuntos = ? AND xdatosadjuntosidespeciesvegetales = ?',
        [photoId, resolvedParams.id]
      );
    ...
    if (action === 'updateMeta' && resumen) {
      await pool.query(
        'UPDATE datosadjuntos SET datosadjuntosresumen = ? WHERE iddatosadjuntos = ? AND xdatosadjuntosidespeciesvegetales = ?',
