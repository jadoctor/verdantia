...
export default function BancalWorkspace() {
  ...
  const triggerNanoBanana = () => {
    ...
    const logs = [
      ...
      `🔎 [0.8s] Identificando especies botánicas y variedades activas en el diseño...`,
      `🌱 [1.1s] Analizando estado de germinación y fenología de los cultivos...`,
      "✍️ [1.4s] Trazando bosquejo artístico e imperfecto a mano alzada en papel de jardín...",
      "🎨 [1.7s] Aplicando acuarela, rayado de lápiz y estacas de tutoría (no germinados)...",
      "✨ [2.0s] Síntesis fotorrealista completada con éxito."
    ];
  ...
  const drawRealisticBancal = (canvas: HTMLCanvasElement) => {
    ...
    // 1. Draw a beautiful warm stone tile background (limestone terrace patio)
    ctx.fillStyle = '#f2efe9'; // premium sunny limestone sand patio
    ctx.fillRect(0, 0, 1000, 1000);

    // Grout lines for the limestone patio tiles
    ctx.strokeStyle = '#e6e3dd';
    ctx.lineWidth = 1.8;
    for (let xG = 0; xG <= 1000; xG += 160) {
      ctx.beginPath();
      ctx.moveTo(xG, 0);
      ctx.lineTo(xG, 1000);
      ctx.stroke();
    }
    for (let yG = 0; yG <= 1000; yG += 160) {
      ctx.beginPath();
      ctx.moveTo(0, yG);
      ctx.lineTo(1000, yG);
      ctx.stroke();
    ...
    // Add subtle stone texture noise to the terrace tiles
    ctx.fillStyle = '#eae7df';
    for (let i = 0; i < 200; i++) {
      const sx = Math.random() * 1000;
      const sy = Math.random() * 1000;
      ctx.beginPath();
      ctx.arc(sx, sy, Math.random() * 2 + 0.8, 0, Math.PI * 2);
      ctx.fill();
    }
    ...
    // Fill soil base area with very dark chocolate compost sustrato loam
    ...
    // Draw the thick planter container frame borders (Slate border CAD style)
    ctx.save();
    ...
    for (let i = 0; i < 900; i++) {
      ctx.fillStyle = Math.random() > 0.5 ? '#271f1b' : '#0e0b09';
    ...
    // ── STAGE 3: PROCEDURE RIVER PEBBLE PATHWAYS (PASILLOS) ──
    if (bancal.bancalesforma !== 'circular') {
      const segments = getBedSegments();
      segments.forEach(s => {
        if (s.type === 'pasillo') {
          ...
          // Generate river pebbles filling the pathway segment beautifully!
          const segW = s.width * scale;
          const segH = s.height * scale;
          const pebbleArea = segW * segH;
          const pebbleCount = Math.min(300, Math.ceil(pebbleArea / 175));
          for (let pIdx = 0; pIdx < pebbleCount; pIdx++) {
            const px = s.xStart + Math.random() * s.width;
            const py = s.yStart + Math.random() * s.height;
            const p = getProjectedCoord(px, py);
            const r = Math.random() * 4 + 7; // 7px to 11px radius pebbles
            drawPebble(p.x, p.y, r);
          }
          ...
          ctx.fillText('PASILLO', (p00.x + p11.x) / 2, (p00.y + p11.y) / 2);
          ctx.restore();
        }
      });
    }

    // ── STAGE 4: WINDING CONTINUOUS PEBBLE PATHS (placedPaths) ──
    placedPaths.forEach(path => {
      ...
      // Sample points along winding path segments and fill with beautiful overlapping pebbles!
      for (let sIdx = 0; sIdx < path.points.length - 1; sIdx++) {
        ...
        const strokeW = (path.widthMeters / w) * (w * scale);
        ...
        for (let step = 0; step <= steps; step++) {
          ...
          const pCount = Math.ceil(strokeW / 12);
          for (let pc = 0; pc < pCount; pc++) {
            const offsetPct = (pc / (pCount - 1 || 1)) - 0.5;
            const perpX = -dy / dist;
            const perpY = dx / dist;
            const offsetPixels = offsetPct * strokeW * 0.85 + (Math.random() - 0.5) * 4;
            
            const pebbleX = scx + perpX * offsetPixels;
            const pebbleY = scy + perpY * offsetPixels;
            const r = Math.random() * 4 + 7;
            drawPebble(pebbleX, pebbleY, r);
          }
        }
      }
      ctx.restore();
    ...
    if (bancal.bancalesforma !== 'circular') {
      ...
      segments.forEach(s => {
        if (s.type !== 'pasillo') {
          ...
          // Slightly warmer, rich dark compost soil for raised camas
    ...
    if (bancal.bancalesforma !== 'circular') {
      ...
      segments.forEach(s => {
        if (s.type !== 'pasillo') {
          ...
          // Render two parallel hoses per crop bed depending on layout orientation
          if (bedAlignment === 'horizontal') {
            const y1 = s.yStart + s.height * 0.3;
            const y2 = s.yStart + s.height * 0.7;
            [y1, y2].forEach(yVal => {
            ...
            const x1 = s.xStart + s.width * 0.3;
            const x2 = s.xStart + s.width * 0.7;
            [x1, x2].forEach(xVal => {
    ...
    crops.forEach(crop => {
      ...
      const deltaX = (spacingXCm / 100);
      const deltaY = (spacingYCm / 100);

      const cols = Math.ceil(Math.sqrt(N));
      const rows = Math.ceil(N / cols);
      const gridW = (cols - 1) * deltaX;
      const gridH = (rows - 1) * deltaY;

      for (let idx = 0; idx < N; idx++) {
        const col = idx % cols;
        const row = Math.floor(idx / cols);

        const cxCenter = (px / 100) * w;
        const cyCenter = (py / 100) * l;

        const cxPlant = cxCenter - gridW / 2 + col * deltaX;
        const cyPlant = cyCenter - gridH / 2 + row * deltaY;
    ...
    renderQueue.forEach(item => {
      ...
      if (item.type === 'crop') {
        ...
        if (status === 'germinacion' || status === 'en_espera') {
          // ── GERMINATION: tutor de caña + cartelera de madera en 3D ──
          ...
          ctx.save();
          ctx.translate(pBase.x + 8, pBase.y + 4);
          ctx.rotate(0.08);
          ctx.fillStyle = '#fef3c7'; 
          ctx.strokeStyle = '#d97706';
          ctx.lineWidth = 1.0;
          ctx.fillRect(0, 0, 32, 14);
          ctx.strokeRect(0, 0, 32, 14);

          ctx.fillStyle = '#78350f';
          ctx.font = 'bold 8px system-ui';
          const tagTxt = (crop.especiesicono || '🌱') + (crop.variedad_nombre || crop.especiesnombre || 'Sem').substring(0, 3);
          ctx.fillText(tagTxt, 3, 10);
          ctx.restore();
  ...
  const updateBedHighlight = (xVal: any, yVal: any) => {
  ...
  const getSnappedCoords = (xVal: any, yVal: any, quantity?: any, spacingXVal?: any, spacingYVal?: any, marginVal?: any) => {
    ...
    if (bedAlignment === 'horizontal') {
      ...
      let numRows = 1;
      if (closestSeg.type === 'cama_bilateral' || closestSeg.height >= 0.9) {
        numRows = 2;
      }
      ...
      let numCols = 1;
      if (closestSeg.type === 'cama_bilateral' || closestSeg.width >= 0.9) {
        numCols = 2;
      }
    ...
    if (draggingCrop) {
      ...
      if (targetCrop) {
        const snapped = getSnappedCoords(
          ...
          targetCrop.cultivoscantidad,
          targetCrop.especiesmarcoplantas,
          targetCrop.especiesmarcofilas,
          targetCrop.especiesmarcomargen
        );
        targetCrop.cultivosposicionx = snapped.x;
        targetCrop.cultivosposiciony = snapped.y;
  ...
  const handleCanvasDrop = async (e: React.DragEvent) => {
    ...
    try {
      ...
      if (source === 'accesorios') {
        ...
        const snapped = getSnappedCoords(x, y, item.cultivoscantidad, item.especiesmarcoplantas, item.especiesmarcofilas);
  ...
  const handleCanvasClick = async (e: React.MouseEvent) => {
    ...
    setPlacingPlant(null);
    const snapped = getSnappedCoords(x, y, 1, tempPlacing.especiesmarcoplantas, tempPlacing.especiesmarcofilas);
  ...
  // Remove/Delete crop action (Desasignar del bancal - Only targets the clicked segment)
  const handleRemoveCrop = async (cropId: number) => {
    try {
      const myCrop = crops.find(c => c.idcultivos === cropId) || allActiveCrops.find(c => c.idcultivos === cropId);
      if (!myCrop) return;

      const res = await fetch(`/api/user/cultivos/${myCrop.idcultivos}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'x-user-email': userEmail! },
        body: JSON.stringify({
      ...
      console.error('Error removing crop from canvas:', err);
    }
  };
  ...
  const handleAutoPlaceCrop = async (cropToPlace: any) => {
    if (!bancal || !showBedGuides || bancal.bancalesforma === 'circular') {
      alert("⚠️ Para poder auto-ubicar con IA es necesario que las guías de camas estén activadas y que el bancal no sea circular.");
      return;
    }
    ...
    if (bedSegments.length === 0) {
      alert("⚠️ No se encontraron camas cultivables definidas en este bancal.");
      return;
    }

    // Filter other crops (excluding current crop)
    const otherCrops = crops.filter(c => c.idcultivos !== cropToPlace.idcultivos);
    ...
    for (let s of bedSegments) {
      ...
      if (bedAlignment === 'horizontal') {
        let numRows = 1;
        if (s.type === 'cama_bilateral' || s.height >= 0.9) {
          numRows = 2;
        }
        const numCols = Math.ceil(N / numRows);
        const gridW = (numCols - 1) * spacingXMeters;
        ...
        const snappedYMeters = s.yStart + s.height / 2;
        const snappedYPct = (snappedYMeters / bedL) * 100;

        // Try candidate X coordinates in this segment, packing from left (s.xStart) to right (s.xStart + s.width)
        const startX = s.xStart + halfGridW + marginMeters;
        const endX = s.xStart + s.width - (halfGridW + marginMeters);

        if (endX < startX) continue; // Doesn't fit in this segment

        for (let xCandidate = startX; xCandidate <= endX + 0.01; xCandidate += 0.05) {
          // Check collision at (xCandidate, snappedYMeters)
          const candidateXPct = (xCandidate / bedW) * 100;
          let collides = false;

          // 1. Check with other crops
          for (let other of otherCrops) {
            ...
            const otherSp = parseFloat(other.especiesmarcoplantas) || 30;
            const otherRadius = (otherSp / 2) / 100;
            const thisRadius = spacingXMeters / 2;

            const dist = Math.sqrt(Math.pow(xCandidate - oxM, 2) + Math.pow(snappedYMeters - oyM, 2));
            if (dist < (otherRadius + thisRadius + 0.05)) {
              collides = true;
              break;
            }
          }
          ...
          // 2. Check with accessories
          for (let acc of placedAccessories) {
            ...
            const dist = Math.sqrt(Math.pow(xCandidate - axM, 2) + Math.pow(snappedYMeters - ayM, 2));
            if (dist < (accR + thisRadius + 0.05)) {
              collides = true;
              break;
            }
          }
        ...
        // bedAlignment === 'vertical'
        let numCols = 1;
        if (s.type === 'cama_bilateral' || s.width >= 0.9) {
          numCols = 2;
        }
        const numRows = Math.ceil(N / numCols);
        const gridH = (numRows - 1) * spacingYMeters;
        ...
        const snappedXMeters = s.xStart + s.width / 2;
        const snappedXPct = (snappedXMeters / bedW) * 100;

        // Try candidate Y coordinates in this segment, packing from top (s.yStart) to bottom (s.yStart + s.height)
        const startY = s.yStart + halfGridH + marginMeters;
        const endY = s.yStart + s.height - (halfGridH + marginMeters);

        if (endY < startY) continue; // Doesn't fit in this segment

        for (let yCandidate = startY; yCandidate <= endY + 0.01; yCandidate += 0.05) {
          ...
          for (let other of otherCrops) {
            ...
            const otherSp = parseFloat(other.especiesmarcoplantas) || 30;
            const otherRadius = (otherSp / 2) / 100;
            const thisRadius = spacingYMeters / 2;

            const dist = Math.sqrt(Math.pow(snappedXMeters - oxM, 2) + Math.pow(yCandidate - oyM, 2));
            if (dist < (otherRadius + thisRadius + 0.05)) {
              collides = true;
              break;
            }
          ...
          for (let acc of placedAccessories) {
            ...
            const dist = Math.sqrt(Math.pow(snappedXMeters - axM, 2) + Math.pow(yCandidate - ayM, 2));
            if (dist < (accR + thisRadius + 0.05)) {
              collides = true;
              break;
            }
    ...
    if (found) {
      try {
        ...
        loadWorkspaceData();
        alert("🤖 [IA Verdantia] ¡Ubicación optimizada con éxito! El cultivo ha sido posicionado de forma agrupada y óptima en una cama de cultivo libre.");
      } catch (err) {
      ...
      for (let s of bedSegments) {
        ...
        // Intentamos encajar el máximo número de plantas posibles (de remaining a 1) en esta cama s
        for (let k = remaining; k >= 1; k--) {
          ...
          if (bedAlignment === 'horizontal') {
            ...
            const numCols = Math.ceil(k / numRows);
            const gridW = (numCols - 1) * spacingXMeters;
            const halfGridW = gridW / 2;
            ...
            const numRows = Math.ceil(k / numCols);
            const gridH = (numRows - 1) * spacingYMeters;
            const halfGridH = gridH / 2;
        ...
        if (bestK > 0) {
          placements.push({ qty: bestK, x: bestX, y: bestY });
          remaining -= bestK;
          virtualCrops.push({
            idcultivos: Date.now() + Math.random(),
            cultivosposicionx: bestX,
            cultivosposiciony: bestY,
            especiesmarcoplantas: spacingX,
            especiesmarcofilas: spacingY,
            cultivoscantidad: bestK
          });
        }
      }
      ...
      if (remaining === 0 && placements.length > 0) {
        try {
          ...
          setSelectedCrop(null);
          loadWorkspaceData();
          alert(`🤖 [IA Verdantia] Distribución Multi-Cama exitosa. El lote de ${N} plantas no cabía completo en una cama, por lo que la IA lo ha repartido armónicamente en ${placements.length} camas ocupando lo justo necesario.`);
        } catch (err) {
          console.error("Error distributing crops across beds:", err);
          alert("⚠️ Ocurrió un error al intentar distribuir los cultivos en las camas.");
        }
      } else {
        alert("⚠️ No se encontró suficiente espacio libre para distribuir el lote de plantas de manera armoniosa en las camas de este bancal, incluso repartiéndolo entre múltiples camas.");
      }
    }
  };
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ margin: 0, fontSize: '0.7rem', color: '#9ca3af' }}>
                          Variedad: {item.variedad_nombre || 'Común'} • Cantidad: {item.cultivoscantidad || 1} • Marco: {item.especiesmarcoplantas}x{item.especiesmarcofilas}cm{item.especiesmarcomargen !== null && item.especiesmarcomargen !== undefined ? ` • Margen: ${item.especiesmarcomargen}cm` : ''}
                        </p>
            ...
            >
              ...
              <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`} style={{ transformOrigin: '250px 250px', transition: isPanning ? 'none' : 'transform 0.15s ease-out' }}>
                  ...
                  return (
                    <g style={{ pointerEvents: 'none' }}>
                      {segments.map((s, idx) => {
                        ...
                        const isOccupied = isCama && crops.some(c => {
                ...
                {crops.map(crop => {
                  ...
                  const deltaPctX = (spacingXMeters / bedW) * 100;
                  const deltaPctY = (spacingYMeters / bedL) * 100;

                  // 1. Obtener todas las camas (strips)
                  const camaBilateral = profile?.camaCultivoBilateral ?? 1.20;
                  const camaUnilateral = profile?.camaCultivoUnilateral ?? 0.75;
                  const pasilloVal = profile?.pasillo ?? 0.50;
                  ...
                  if (bancal?.bancalesforma !== 'circular') {
                    if (bedAlignment === 'horizontal') {
                      // Filtrar senderos horizontales del usuario
                      const horizontalPaths = placedPaths.filter(p => {
                        if (!p.points || p.points.length < 2) return false;
                        const first = p.points[0];
                        const last = p.points[p.points.length - 1];
                      ...
                      if (bedL > currentStart + 0.05) {
                        intervals.push({ start: currentStart, end: bedL, topIsPath: currentStart > 0, bottomIsPath: false });
                      }
                      ...
                      // Filtrar senderos verticales del usuario
                      const verticalPaths = placedPaths.filter(p => {
                        if (!p.points || p.points.length < 2) return false;
                        const first = p.points[0];
                        const last = p.points[p.points.length - 1];
                      ...
                      if (bedW > currentStart + 0.05) {
                        intervals.push({ start: currentStart, end: bedW, leftIsPath: currentStart > 0, rightIsPath: false });
                      }
                  ...
                  // 2. Encontrar a qué cama pertenece el cultivo (el más cercano a px/py)
                  let myBed = bedStrips[0];
                  if (bedStrips.length > 0) {
                    const coordVal = bedAlignment === 'horizontal' ? (py / 100) * bedL : (px / 100) * bedW;
                    let minDist = Infinity;
                    bedStrips.forEach(s => {
                  ...
                  // 3. Decidir número de filas/columnas en base a la anchura de la cama
                  let numCols = 1;
                  let numRows = N;
                  
                  if (bancal?.bancalesforma !== 'circular' && myBed) {
                    if (bedAlignment === 'horizontal') {
                      if (myBed.type === 'cama_bilateral' || myBed.size >= 0.9) {
                        numRows = 2;
                      } else {
                        numRows = 1;
                      }
                  ...
                  return (
                    <g key={crop.idcultivos}>
                      {plants.map(p => {
                        ...
                        return (
                          <g key={p.idx}>
                            {/* Visual Planting Frame Guide (Marco de Plantación - Only visible on selection or collision) */}
                            ...
                            {/* Measurement Capsule Label */}
                            {showPlantingFrames && isSelected && (
                              <g style={{ pointerEvents: 'none' }}>
                                {/* Soft dark glassmorphic capsule background */}
                                <rect
                                  x={p.cx - 28}
                                  y={p.cy - frameH / 2 - 14}
                                  width="56"
                                ...
                                <text
                                  ...
                                  style={{ userSelect: 'none' }}
                                >
                                  {spacingXCm}x{spacingYCm} cm
                                </text>
                              </g>
                            )}

                            {/* Collision / Spacing Ring (Only visible on selection or collision) */}
                            {showMeasures && (isSelected || hasCollision) && (
                              <circle
                                cx={p.cx} cy={p.cy} r={r}
                                fill={hasCollision ? '#ef4444' : '#10b981'}
                                fillOpacity={hasCollision ? 0.15 : 0.08}
                            ...
                            {/* Collection indicator (Only on the first plant of the batch to prevent clutter) */}
                            {p.idx === 0 && badgeR >= 10 && (
                              <g style={{ pointerEvents: 'none' }}>
                                <circle cx={p.cx + badgeR * 0.8} cy={p.cy - badgeR * 0.8} r="5" fill="#111827" stroke="white" strokeWidth="0.5" />
                                <text x={p.cx + badgeR * 0.8} y={p.cy - badgeR * 0.8 + 1.5} textAnchor="middle" fill="white" fontSize="5" fontWeight="bold">
            ...
            {selectedPath ? (
              ...
              <>
                ...
                <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: '10px', flexShrink: 0 }}>
                  ...
                  {/* Action: Remove from Bed (keeps in inventory) */}
                  <button 
                    ...
                    onMouseLeave={e => e.currentTarget.style.background = '#7f1d1d'}
                  >
                    <Trash2 size={16} /> Quitar del Bancal
                  </button>
                </div>
              </>
            ) : (
              ...
              {nanoBananaLoading ? (
                ...
                <>
                  ...
                  <div style={{
                    ...
                    maxWidth: '480px', height: '480px', display: 'flex',
