...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const safeTitle = title || 'Documento IA';
    ...
    const [result] = await pool.query(
      ...
      [safeTitle, url, total + 1, idlabores, safeTitle, safeSummary, safeApuntes, safeAutores, safeIdentificacion]
    );
    ...
    return NextResponse.json({
      ...
      pdf: {
        ...
        ruta: url,
        nombreOriginal: safeTitle,
        titulo: safeTitle,
        resumen: safeSummary,
        apuntes: safeApuntes,
        autores: safeAutores,
        identificacion: safeIdentificacion,
        portada: null
      }
    });
