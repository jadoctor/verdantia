...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const [result] = await pool.query(
      ...
      [file.name, storagePath, total + 1, idlabores, fileSize, generatedTitle, generatedSummary, generatedApuntes, generatedAutores, generatedIdentificacion]
    );
    ...
    if (base64Cover) {
      ...
      if (!docTitle) {
        const [pdfRows] = await pool.query(
          "SELECT datosadjuntostitulo FROM datosadjuntos WHERE iddatosadjuntos = ? AND xdatosadjuntosidlabores = ?",
          [pdfId, idlabores]
        );
      ...
      const storagePath = `uploads/labores_pdfs_covers/${cleanTitle}_${pdfId}_${Date.now()}.jpg`;
      const { uploadToStorage } = await import('@/lib/firebase/storage');
