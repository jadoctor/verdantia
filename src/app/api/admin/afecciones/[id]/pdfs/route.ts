...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos, datosadjuntosruta, datosadjuntosnombreoriginal, datosadjuntostitulo, datosadjuntosresumen, datosadjuntosapuntes, datosadjuntosportada, datosadjuntosactivo
       FROM datosadjuntos 
       WHERE xdatosadjuntosidafecciones = ? 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const filename = `variedad_${idafecciones}_${Date.now()}.pdf`;
    const storagePath = `uploads/afecciones_pdfs/${filename}`;
    ...
    const [countResult] = await pool.query(
      "SELECT COUNT(*) as total FROM datosadjuntos WHERE xdatosadjuntosidafecciones = ? AND datosadjuntostipo = 'documento' AND datosadjuntosactivo = 1",
      [idafecciones]
    );
    ...
    const [result] = await pool.query(
      ...
      [file.name, storagePath, total + 1, idafecciones, fileSize, generatedTitle, generatedSummary, generatedApuntes]
    );
    ...
    console.error('[Afeccion PDFs API] Upload error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { pdfId, titulo, resumen, apuntes, portada, base64Cover, activo } = await request.json();
    ...
    if (base64Cover) {
      ...
      if (!docTitle) {
        const [pdfRows] = await pool.query(
          "SELECT datosadjuntostitulo FROM datosadjuntos WHERE iddatosadjuntos = ? AND xdatosadjuntosidafecciones = ?",
          [pdfId, idafecciones]
        );
      ...
      const storagePath = `uploads/afecciones_pdfs_covers/${cleanTitle}_${pdfId}_${Date.now()}.jpg`;
      const { uploadToStorage } = await import('@/lib/firebase/storage');
    ...
    updateQuery += " WHERE iddatosadjuntos = ? AND xdatosadjuntosidafecciones = ? AND datosadjuntostipo = 'documento'";
    updateParams.push(pdfId, idafecciones);

    await pool.query(updateQuery, updateParams);

    return NextResponse.json({ success: true, portada: finalPortada });
  } catch (error: any) {
    console.error('[Afeccion PDFs PUT API] error:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
