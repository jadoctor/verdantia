  ...
  try {
    const [rows] = await pool.query(
      `SELECT iddatosadjuntos as id, datosadjuntosruta as ruta, 
       ...
       FROM datosadjuntos 
       WHERE xdatosadjuntosidvariedades = ? AND datosadjuntostipo = 'imagen' AND datosadjuntosactivo = 1 
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!variedadNombre) {
      const [variedadResult] = await pool.query("SELECT variedadesnombre FROM variedades WHERE idvariedades = ?", [idvariedades]);
      variedadNombre = (variedadResult as any[])[0]?.variedadesnombre || 'Variedad';
    }
    ...
    const [result] = await pool.query(
      ...
      [
        ...
        total + 1, idvariedades, fileSize, initialResumen
      ]
    );
