...
interface AlimentacionItem {
  idespeciesconsumidores: number;
  xespeciesconsumidoresidespecies: number;
  xespeciesconsumidoresidconsumidores: number;
  especiesconsumidoresesapto: number;
  especiesconsumidorespartes: string;
  especiesconsumidoresnotas: string;
  especiesnombre: string;
  especiesicono: string;
  primary_photo_ruta?: string;
  especiesconsumidoresemoji?: string;
}
...
export default function ConsumidorEditPage() {
  ...
  const [photoSaveStatus, setPhotoSaveStatus] = useState<'idle' | 'saving'>('idle');
  const editorDragRef = useRef<{ dragging: boolean; startX: number; startY: number; startPosX: number; startPosY: number }>({ dragging: false, startX: 0, startY: 0, startPosX: 50, startPosY: 50 });
  ...
  const [formData, setFormData] = useState<ConsumidorForm>({
    consumidoresnombre: '',
    consumidoresicono: '👤',
    consumidoresdescripcion: '',
    consumidoresactivo: 1
  });
  
  const [initialData, setInitialData] = useState<ConsumidorForm | null>(null);
  ...
  const fetchConsumidor = async (email: string) => {
    try {
      const res = await fetch(`/api/admin/consumidores/${params.id}`, { headers: { 'x-user-email': email } });
  ...
  const fetchAlimentacion = async (email: string) => {
    try {
      const res = await fetch(`/api/admin/consumidores/${params.id}/alimentacion`, { headers: { 'x-user-email': email } });
      const data = await res.json();
  ...
  const fetchPhotos = async (email: string) => {
    try {
      const res = await fetch(`/api/admin/consumidores/${params.id}/photos`, { headers: { 'x-user-email': email } });
      const data = await res.json();
      if (data.photos) {
  ...
  const autoSave = useCallback(async (dataToSave: ConsumidorForm) => {
    if (isNew || !userEmail) return;
    setSaveStatus('saving');
    try {
      const res = await fetch(`/api/admin/consumidores/${params.id}`, {
        method: 'PUT',
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) router.push(`/dashboard/admin/consumidores/${data.id}`);
  ...
  const buildPromptPreview = (conceptOverride?: string) => {
    const nombre = formData.consumidoresnombre || 'animal';
  ...
  const generateAiImage = async () => {
    if (!formData.consumidoresnombre) {
    ...
    try {
      const body: any = {
        tipoEntidad: 'consumidor',
        especieNombre: formData.consumidoresnombre,
  ...
  const uploadAiImage = async () => {
    ...
    try {
      ...
      const descBase = aiImageDescription || formData.consumidoresnombre || 'animal';
      ...
      const saveRes = await fetch(`/api/admin/consumidores/${params.id}/photos`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-user-email': userEmail },
        body: JSON.stringify({
  ...
  }, [userEmail, formData.consumidoresnombre, photos.length]);
  ...
  const handleFileUpload = async (files: FileList | File[] | null) => {
    ...
    try {
      ...
      for (let i = 0; i < filesArray.length; i++) {
        ...
        await fetch(`/api/admin/consumidores/${params.id}/photos`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'x-user-email': userEmail },
  ...
  const handleDeletePhoto = async (photoId: number) => {
    ...
    await fetch(`/api/admin/consumidores/${params.id}/photos?id=${photoId}`, {
  ...
  const openPhotoEditor = (photo: PhotoItem) => {
    ...
    } catch { setEditorX(50); setEditorY(50); setEditorZoom(100); setEditorBrightness(100); setEditorContrast(100); setEditorStyle(''); setEditorSeoAlt(''); }
  ...
  const savePhotoEdits = async () => {
    ...
    await fetch(`/api/admin/consumidores/${params.id}/photos`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json', 'x-user-email': userEmail },
      body: JSON.stringify({ action: 'updateMeta', photoId: editingPhoto.id, resumen })
    });
  ...
  const onEditorMouseDown = (e: React.MouseEvent) => {
    ...
    const onMove = (ev: MouseEvent) => {
      ...
      setEditorX(Math.max(0, Math.min(100, editorDragRef.current.startPosX - (ev.clientX - editorDragRef.current.startX) * sensitivity)));
      setEditorY(Math.max(0, Math.min(100, editorDragRef.current.startPosY - (ev.clientY - editorDragRef.current.startY) * sensitivity)));
    };
    const onUp = () => { editorDragRef.current.dragging = false; window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp); };
  ...
  return (
    <div style={{ width: '100%', boxSizing: 'border-box', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
          ...
          <div className="photo-editor-content" onClick={e => e.stopPropagation()}>
            <div className="photo-editor-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                ...
                >
                  {photoSaveStatus === 'saving' ? '⏳ Guardando...' : '💾 Smart Save'}
                </button>
              </div>
            </div>
          ...
          <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '600px', display: 'flex', flexDirection: 'column', maxHeight: '90vh', overflow: 'hidden', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.5)' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #e2e8f0', background: 'linear-gradient(to right, #f8fafc, #f1f5f9)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                ...
                <p style={{ margin: '4px 0 0', fontSize: '0.85rem', color: '#64748b' }}>Animal de granja: <strong>{formData.consumidoresnombre || 'Sin nombre'}</strong></p>
              </div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px' }}>
        ...
        <button onClick={() => router.push('/dashboard/admin/consumidores')} style={{ background: '#f1f5f9', border: '1px solid #cbd5e1', padding: '8px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: 500, color: '#334155' }}>
          🔙 Volver a Animales de granja
        </button>
      </div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #f59e0b, #d97706)', borderRadius: '14px', padding: '12px 20px', marginBottom: '24px', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: 800, display: 'flex', alignItems: 'center', gap: '12px' }}>
            {formData.consumidoresicono} {formData.consumidoresnombre || 'Nuevo Animal de granja'}
          </h1>
          <p style={{ margin: '4px 0 0', opacity: 0.9, fontSize: '0.85rem' }}>
            ✏️ Editar Animal de granja · ID del Registro: {isNew ? 'Nuevo' : params.id}
          </p>
        </div>
      ...
      {/* Estado Global */}
      <div style={{ background: formData.consumidoresactivo === 1 ? '#ecfdf5' : '#f1f5f9', borderRadius: '12px', padding: '12px 20px', marginBottom: '20px', border: `1px solid ${formData.consumidoresactivo === 1 ? '#10b981' : '#cbd5e1'}`, display: 'flex', alignItems: 'center', gap: '12px', transition: 'all 0.3s' }}>
        <input type="checkbox" name="consumidoresactivo" checked={formData.consumidoresactivo === 1} onChange={handleChange} style={{ width: '22px', height: '22px', accentColor: '#10b981', cursor: 'pointer' }} />
        <span style={{ fontWeight: 700, fontSize: '0.95rem', color: formData.consumidoresactivo === 1 ? '#065f46' : '#475569' }}>
          {formData.consumidoresactivo === 1 ? '🟢 Activo (Visible en todo el sistema)' : '🔴 Inactivo (Oculto)'}
        </span>
      </div>
      ...
      {photos.length > 0 ? (() => {
        ...
        return (
          <div style={{
            ...
            >
              {heroPhoto && (
                <div
                  onClick={() => setLightboxPhoto({ url: getMediaUrl(heroPhoto.ruta), nombre: formData.consumidoresnombre })}
                ...
                >
                  ...
                  <img key={heroPhoto.id} src={getMediaUrl(heroPhoto.ruta)}
                    alt={heroMeta.seo_alt || formData.consumidoresnombre}
                    style={{
        ...
        <div style={{ padding: '20px 24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          {formData.consumidoresicono && <span style={{ fontSize: '2.5rem' }}>{formData.consumidoresicono}</span>}
      ...
      <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #e2e8f0', overflow: 'hidden', marginBottom: '24px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)' }}>
        ...
        <div onClick={() => setIsDatosExpanded(!isDatosExpanded)} style={{ padding: '16px 24px', background: isDatosExpanded ? 'transparent' : '#f8fafc', borderBottom: isDatosExpanded ? '1px solid #e2e8f0' : 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', transition: 'background 0.2s' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {!isDatosExpanded && formData.consumidoresnombre && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#e2e8f0', padding: '4px 12px', borderRadius: '20px', fontSize: '0.85rem', color: '#475569' }}>
                <span>{formData.consumidoresicono}</span>
                <span style={{ fontWeight: 600 }}>{formData.consumidoresnombre}</span>
                <span>({formData.consumidoresactivo ? 'Activo' : 'Inactivo'})</span>
              </div>
            )}
          </div>
          ...
          <div style={{ display: datosSubTab === 'datos' ? 'block' : 'none', padding: '24px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontWeight: 600, fontSize: '0.9rem', color: '#334155' }}>Nombre del Animal de granja *</label>
                <input name="consumidoresnombre" value={formData.consumidoresnombre} onChange={handleChange} placeholder="Ej: Gallina" style={{ padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                <label style={{ fontWeight: 600, fontSize: '0.9rem', color: '#334155' }}>Icono (Emoji)</label>
                <input name="consumidoresicono" value={formData.consumidoresicono} onChange={handleChange} placeholder="Ej: 🐔" style={{ padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem' }} />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', gridColumn: '1 / -1' }}>
                <label style={{ fontWeight: 600, fontSize: '0.9rem', color: '#334155' }}>Descripción Genérica</label>
                <textarea name="consumidoresdescripcion" value={formData.consumidoresdescripcion} onChange={handleChange} placeholder="Añade notas biológicas sobre este animal de granja (herbívoro, omnívoro...)" style={{ padding: '10px', borderRadius: '8px', border: '1px solid #cbd5e1', fontSize: '1rem', minHeight: '100px', resize: 'vertical' }} />
              </div>
            </div>
            {isNew && (
              <div style={{ marginTop: '24px', display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid #e2e8f0', paddingTop: '20px' }}>
                <button onClick={handleCreate} disabled={saveStatus === 'saving' || !formData.consumidoresnombre} style={{ background: '#d97706', color: 'white', border: 'none', padding: '12px 24px', borderRadius: '8px', fontWeight: 'bold', cursor: 'pointer', fontSize: '1rem' }}>
                  {saveStatus === 'saving' ? 'Guardando...' : 'Crear Animal de granja'}
                </button>
              </div>
            )}
          </div>
                    ...
                    {uploading ? (
                      ...
                      <div className="drop-zone-content" style={{ display: 'flex', flexDirection: 'column', height: '100%', justifyContent: 'center', alignItems: 'center', gap: '8px' }}>
                        <div className="drop-zone-buttons" style={{ display: 'flex', flexDirection: 'column', gap: '8px', width: '100%', alignItems: 'center' }}>
                          <label
                            htmlFor="upload-consumidor-photos"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            {!isAlimentacionExpanded && (
              <div style={{ display: 'flex', gap: '12px', fontSize: '0.85rem' }}>
                <span style={{ background: '#dcfce7', color: '#166534', padding: '4px 10px', borderRadius: '20px', fontWeight: 600 }}>🟢 {alimentacion.filter(a => a.especiesconsumidoresesapto === 1).length} Permitidos</span>
                <span style={{ background: '#fef3c7', color: '#b45309', padding: '4px 10px', borderRadius: '20px', fontWeight: 600 }}>🟡 {alimentacion.filter(a => a.especiesconsumidoresesapto === 2).length} Con Moderación</span>
                <span style={{ background: '#fee2e2', color: '#991b1b', padding: '4px 10px', borderRadius: '20px', fontWeight: 600 }}>🔴 {alimentacion.filter(a => a.especiesconsumidoresesapto === 0).length} Tóxicos</span>
              </div>
            )}
          </div>
          <div style={{ color: '#64748b', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: 600 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>
            ...
            <div style={{ background: '#f0fdf4', borderRadius: '12px', padding: '20px', border: '1px solid #bbf7d0' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {alimentacion.filter(a => a.especiesconsumidoresesapto === 1).length === 0 ? (
                  <p style={{ color: '#15803d', fontSize: '0.9rem', margin: 0, fontStyle: 'italic' }}>No hay alimentos permitidos registrados.</p>
                ) : (
                  alimentacion.filter(a => a.especiesconsumidoresesapto === 1).map(item => (
                    <div key={item.idespeciesconsumidores} style={{ background: 'white', padding: '12px', borderRadius: '8px', border: '1px solid #dcfce7', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', display: 'flex', gap: '12px', alignItems: 'center' }}>
                      <div style={{ width: '80px', height: '80px', borderRadius: '10px', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f0fdf4', border: '1px solid #bbf7d0', overflow: 'hidden', cursor: item.primary_photo_ruta ? 'zoom-in' : 'default' }}
                        onClick={() => item.primary_photo_ruta && setLightboxPhoto({ url: getMediaUrl(item.primary_photo_ruta), nombre: item.especiesnombre })}
                        title={item.primary_photo_ruta ? 'Ver foto completa' : ''}>
                        {item.primary_photo_ruta ? (
                          <img src={getMediaUrl(item.primary_photo_ruta)} alt={item.especiesnombre} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        ) : (
                          <span style={{ fontSize: '2.2rem' }}>{item.especiesicono || '🌿'}</span>
                        )}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                          <strong style={{ color: '#166534', fontSize: '1rem' }}>{item.especiesnombre}</strong>
                        </div>
                        <div style={{ fontSize: '0.85rem', color: '#15803d', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          {item.especiesconsumidorespartes && <span><strong>Partes:</strong> {item.especiesconsumidoresemoji || '🌱'} {item.especiesconsumidorespartes}</span>}
                          {item.especiesconsumidoresnotas && <span><strong>Notas:</strong> {item.especiesconsumidoresnotas}</span>}
                        </div>
                      </div>
                      <button onClick={() => router.push(`/dashboard/admin/especies/${item.xespeciesconsumidoresidespecies}?from=consumidores&fromId=${params.id}&fromName=${encodeURIComponent(formData.consumidoresnombre || 'Animal de granja')}&tab=consumos`)}
            ...
            <div style={{ background: '#fffbeb', borderRadius: '12px', padding: '20px', border: '1px solid #fde68a' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {alimentacion.filter(a => a.especiesconsumidoresesapto === 2).length === 0 ? (
                  <p style={{ color: '#b45309', fontSize: '0.9rem', margin: 0, fontStyle: 'italic' }}>No hay alimentos con moderación registrados.</p>
                ) : (
                  alimentacion.filter(a => a.especiesconsumidoresesapto === 2).map(item => (
                    <div key={item.idespeciesconsumidores} style={{ background: 'white', padding: '12px', borderRadius: '8px', border: '1px solid #fde68a', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', display: 'flex', gap: '12px', alignItems: 'center' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                          <strong style={{ color: '#92400e', fontSize: '1rem' }}>{item.especiesnombre}</strong>
                        </div>
            ...
            <div style={{ background: '#fef2f2', borderRadius: '12px', padding: '20px', border: '1px solid #fecaca' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {alimentacion.filter(a => a.especiesconsumidoresesapto === 0).length === 0 ? (
                  <p style={{ color: '#b91c1c', fontSize: '0.9rem', margin: 0, fontStyle: 'italic' }}>No hay alimentos tóxicos registrados.</p>
                ) : (
                  alimentacion.filter(a => a.especiesconsumidoresesapto === 0).map(item => (
                    <div key={item.idespeciesconsumidores} style={{ background: 'white', padding: '12px', borderRadius: '8px', border: '1px solid #fee2e2', boxShadow: '0 2px 4px rgba(0,0,0,0.02)', display: 'flex', gap: '12px', alignItems: 'center' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                          <strong style={{ color: '#991b1b', fontSize: '1rem' }}>{item.especiesnombre}</strong>
                        </div>
                        <div style={{ fontSize: '0.85rem', color: '#b91c1c', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                          {item.especiesconsumidorespartes && <span><strong>Partes tóxicas:</strong> {item.especiesconsumidoresemoji || '🌱'} {item.especiesconsumidorespartes}</span>}
                          {item.especiesconsumidoresnotas && <span><strong>Notas:</strong> {item.especiesconsumidoresnotas}</span>}
                        </div>
                      </div>
                      <button onClick={() => router.push(`/dashboard/admin/especies/${item.xespeciesconsumidoresidespecies}?from=consumidores&fromId=${params.id}&fromName=${encodeURIComponent(formData.consumidoresnombre || 'Animal de granja')}&tab=consumos`)}
